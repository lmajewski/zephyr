 ...
 *
 * @param iface Network interface (master)
 * @param dsa_send_t Pointer to master interface send method
 *
 * Returns:
 *  - 0 if ok, < 0 if error
 */
...
struct dsa_context {
	...
	/** The 'switch_id = X' - which is the reg=<X> from DTS is used to
	 * distinct between many connected switches.
	 */
	uint8_t switch_id;
...
/**
 * @brief Structure to provide DSA switch hardware related (i.e. configuration)
 * api callbacks
 */
struct dsa_switch_hw_api {
	int w;
};
